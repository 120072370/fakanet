...
$is_api='';
$cons='is_status=1 ';
$cons.=' and `fromid`='.$_SESSION['login_userid']; 
if($do=='search'){
	$fdate=_G('fdate');
	$tdate=_G('tdate');
...
if($st!='' && $kw!=''){
	switch($st){
		...
		break;
	  case 'st5':
		  $cons.=" AND orderid in(SELECT orderid FROM ".DB_PREFIX."selllist WHERE cardid=(SELECT id FROM ".DB_PREFIX."goods WHERE cardnum='$kw'))";
	  break;
		default:
			$kw=strlen($kw)==20 ? substr($kw,0,16) : $kw;
			$cons.=" AND orderid='$kw'";
...
if($action==''){
	...
	$totalPrice=$db->query("SELECT sum(price) as price FROM ".DB_PREFIX."buylist A $t2_cons $cons");
	$totalPrice=$db->fetch_array($totalPrice);
	$totalPrice1=$db->query("SELECT sum(price) as price FROM ".DB_PREFIX."buylist A $t2_cons $cons1");
    ...
    $totalPrice3=$db->query("SELECT unpaid as price FROM ".DB_PREFIX."usermoney where userid=".$_SESSION['login_userid']);
	$totalPrice3=$db->fetch_array($totalPrice3);
	...
	require View::getView("tg_orders");
