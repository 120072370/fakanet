...
if($paytype=='bank' || $paytype=='alipay' || $paytype=='tenpay'||$paytype=='weixin'||$paytype=='wxapp'||$paytype=='qqcode'||$paytype=='aliwap'){
	if($pid=='ALIPAY'){
	    $payid=25;
	} else if($pid=='TENPAY'){
		$payid=26;
...
if($paytype=='bank' || $paytype=='alipay' || $paytype=='tenpay'|| $paytype=='weixin'|| $paytype=='ALIWAP'|| $paytype=='QQCODE'){
	...
	//create redirect url
    //  $redirectUrl='http://w.weigwl.cn/pay/'.$accessType.'_bank/send_'.$accessType.'.php?price='.$paymoney.'&orderid='.$orderid.'&payorderid='.$payorderid.'&pid='.$pid;
    
    $redirectUrl='http://'.$config['wxpay_host'].'/pay/'.$accessType.'_bank/send_'.$accessType.'.php?price='.$paymoney.'&orderid='.$orderid.'&payorderid='.$payorderid.'&pid='.$pid;
    
	require View::getView('wap');//售卡页面
	View::Output();
	...
	if($accessType!='ofpay'){
        ...
        //create redirect url
        $redirectUrl='http://'.$config['wxpay_host'].'/pay/'.$accessType.'_card/send_'.$accessType.'.php';
