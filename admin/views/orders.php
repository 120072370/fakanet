...
<input type="button" onclick="Boxy.load('?action=deldata&fdate=<?php echo $fdate ?>    &tdate=<?php echo $tdate ?>    &is_status=<?php echo $is_status ?>    &channelid=<?php echo $channelid ?>    &searchtype=<?php echo $searchtype ?>    &searchtext=<?php echo $searchtext ?>    ',{title:'清除订单记录',unloadOnHide:true})"  class="button_bg_2" value="清除订单" />&nbsp;
<input type="button" onclick="Boxy.load('?action=closeOrder&fdate=<?php echo $fdate ?>    &tdate=<?php echo $tdate ?>    &is_status=<?php echo $is_status ?>    &channelid=<?php echo $channelid ?>    &searchtype=<?php echo $searchtype ?>    &searchtext=<?php echo $searchtext ?>    ',{title:'批量关闭订单记录',unloadOnHide:true})"  class="button_bg_2" value="关闭订单" />&nbsp;
<?php
...
<?php if($row['is_status']==1): ?>
    <a <?php echo $row['is_receive'] ? '' : 'style="color:blue;"' ?> href="javascript:void(0)" onclick="Boxy.load('?action=getgoodinfo&orderid=<?php echo $row['orderid'] ?>',{title:'<?php echo $row['goodname'] ?>',unloadOnHide:true})"><?php echo $row['goodname'] ?></a>
<?php else: ?>
...
<td class="center"><?php echo number_format($row['income_money'],3,'.','') ?></td>
<td class="center"><?php echo number_format($row['platformincome_money'],3,'.','') ?></td>
