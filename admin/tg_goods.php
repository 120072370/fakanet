...
if($action==''){
    ...
    $result=$db->query("SELECT count(*) FROM ".DB_PREFIX."goodlist where is_tg=1");
	$row=$db->fetch_array($result);
	$totalsize=$row[0];
	$lists=array();
	$result=$db->query("SELECT a.*,b.username FROM ".DB_PREFIX."goodlist as a inner join ".DB_PREFIX."users as b on a.userid=b.id where a.is_tg=1 order by a.id LIMIT $offset,$pagesize");
    if($totalsize>0){
...
if($action=='tghyfc'){
	$db=Mysql::getInstance();
	$result=$db->query("update ".DB_PREFIX."goodlist set tghyfc=".trim($_REQUEST['tghyfc'])." where id=".$_REQUEST['id']);
...
if($action=='ptfc'){
	$db=Mysql::getInstance();
	$result=$db->query("update ".DB_PREFIX."goodlist set ptfc=".trim($_REQUEST['ptfc'])." where id=".$_REQUEST['id']);
	header("Content-Type:application/json; charset=utf-8");
	if($result){
	    $re=array(
			'status'=>1
