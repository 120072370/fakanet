...
if($action==''){
    ...
    if($totalsize>0){
	    while($row=$db->fetch_array($result)){
			...
			//金额
			//$res=$db->query("SELECT sum(money),sum(realmoney),sum(realmoney*price),sum(realmoney*(platformPrice-price)) FROM ".DB_PREFIX."orderlist WHERE channelid<>0 AND orderid LIKE '".$row['orderid']."%'");
