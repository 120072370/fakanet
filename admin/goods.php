...
if($action==''){
    ...
    $result=$db->query("SELECT count(*) FROM ".DB_PREFIX."goodlist");
	$row=$db->fetch_array($result);
	$totalsize=$row[0];
	$lists=array();
	
	$where='where 1 ';
	if($_REQUEST['is_state']!=''){
	    $where=" and a.is_state=".intval($_REQUEST['is_state']);
	}
	if($_REQUEST['username']!=''){
	    $where=" and b.username like '%".$_REQUEST['username']."%'";
	}
	
	$result=$db->query("SELECT a.*,b.username,c.catename FROM ".DB_PREFIX."goodlist as a inner join ".DB_PREFIX."users as b on a.userid=b.id inner join ".DB_PREFIX."goodcate as c on a.cateid=c.id {$where} order by a.id LIMIT $offset,$pagesize");
    if($totalsize>0){
...
if($action=='shenhe'){
	...
	$is_state=intval($_REQUEST['is_state']);
	$data=array(
		'is_state'=>$is_state,
	);
	$wddb->autoExecute(DB_PREFIX."goodlist", $data,'update',"id={$id}");
