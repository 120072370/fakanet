<?php
//$wddb =$wddb= new cls_mysql($cfg_dbhost, $cfg_dbuser, $cfg_dbpwd, $cfg_dbname,$cfg_db_language);

if (!class_exists('cls_mysql', false)) {
	class cls_mysql{
		var $link_id    = NULL;
		...
		var $cache_data_dir = '/data/wdcache/sqlcache/';
	...
	/* 仿真 Adodb 函数 */
		...
		function getSqlCacheData($sql, $cached = '')
		{
			...
			mkdirs($this->root_path . $this->cache_data_dir .$pathstr1);
			$result = array();
		...
		function get_table_name($query_item)
		{
			...
			$prefix="#@__";
			$query_item = str_replace($prefix, $GLOBALS['cfg_dbprefix'], $query_item);
			$table_names = array();
