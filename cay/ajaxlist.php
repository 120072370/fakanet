...
if($action=='getgoodInfo'){
	...
	if($goodList){
        ...
        $min_quantity=$goodList['min_quantity'];
	}
	...
	echo $price.','.$goodInvent.'<input type="hidden" name="kucun" value="'.$kucun.'">,'.$is_discount.','.$is_coupon.','.$is_pwdforsearch.','.$limit_quantity.','.$min_quantity;
	exit;
}
...
if($action=='goodDiscountDesc'){
	...
	if($goodDiscount){
		//$str='
        //<style type="text/css">
        //.goodDiscountDesc{ border: none;border-collapse: collapse; line-height:1.5em}
        //.goodDiscountDesc td,.goodDiscountDesc th{ padding: 3px;}
        //</style>
        //<table class="goodDiscountDesc"  border="0" cellspacing="0" cellpadding="0" style="width:200px;">
        //    <colgroup>
        //        <col width="100" />
        //        <col width="100" />
        //    </colgroup>
        //    <tr><th style="text-align:center">购买数量</th><th style="text-align:center">优惠价格</th></tr>';
	    foreach($goodDiscount as $key=>$val){
		    //$str.='<tr><td style="text-align:center">'.$val['dis_quantity'].'</td><td style="text-align:center">'.$val['dis_price'].'</td></tr>';
            $str.='<span>满'.$val['dis_quantity'].'件,优惠价格:'.$val['dis_price'].'元</span>';
		}
		//$str.='</table>';
		echo $str;
	}
